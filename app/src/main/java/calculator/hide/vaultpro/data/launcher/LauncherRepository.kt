...
import calculator.hide.vaultpro.data.launcher.db.HiddenAppEntity
import calculator.hide.vaultpro.data.launcher.db.LauncherDatabase
...
class LauncherRepository(context: Context) {
    ...
    suspend fun loadHomeData(): LauncherHomeData {
        ...
        val desktopEntities = db.desktopItemDao().getAll()
            .filter { it.key in publicKeys && it.key !in hiddenKeys }
    ...
    suspend fun addToDock(app: LaunchableApp): Boolean {
        ...
        val position = (0 until LauncherConstants.DOCK_SIZE).firstOrNull { it !in usedPositions }
            ?: return false
