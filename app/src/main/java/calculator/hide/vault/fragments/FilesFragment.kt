...
import calculator.hide.vault.data.VaultDatabase
import calculator.hide.vault.data.VaultItem
import calculator.hide.vault.import.SafImporter
import calculator.hide.vault.security.CryptoManager
import kotlinx.coroutines.launch

/**
 * Fragment for displaying files
 */
class FilesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = VaultItemAdapter(ItemType.FILE, false,
            onItemClick = { item ->
                openPreview(item)
            },
            onExportClick = { item ->
        ...
        lifecycleScope.launch {
            database.vaultDao().getItemsByType(ItemType.FILE).collect { items ->
    ...
    fun importFile(uri: Uri, mode: calculator.hide.vault.import.SafImporter.ImportMode) {
        lifecycleScope.launch {
            try {
                ...
                if (result.item != null) {
                    ...
                    when (result.deleteResult) {
                        calculator.hide.vault.import.SafImporter.DeleteResult.SUCCESS -> {
                        ...
                        calculator.hide.vault.import.SafImporter.DeleteResult.NEED_USER_CONFIRM -> {
                            (requireActivity() as? calculator.hide.vault.activities.VaultActivity)?.requestDeleteConfirmation(uri)
                        }
                        else -> {
                            android.widget.Toast.makeText(
                                requireContext(),
    ...
    fun exportItem(item: VaultItem) {
        lifecycleScope.launch {
            try {
                val exportManager = calculator.hide.vault.export.ExportManager(requireContext(), calculator.hide.vault.security.CryptoManager(requireContext()))
    ...
    private fun openPreview(item: VaultItem) {
        val intent = android.content.Intent(requireContext(), calculator.hide.vault.activities.PreviewActivity::class.java).apply {
